...
def quote():
    ...
    filename = get_filename(data_dir)

    with open(filename) as json_data:
        try:
            quotes = json.load(json_data)
        except ValueError:
            print_files_error_info(filename)
            return

        ran_no = random.randint(1, len(quotes["data"])) - 1
        if "quote" in quotes["data"][ran_no]:
            print_quote(quotes, ran_no)
        else:
            print_data_error_info(filename, quotes, ran_no)
...
def print_data_error_info(filename, quotes, ran_no):
    ...
    print("Possibly this json file uses capitalized initial letter in its key.")
    ...
    print("Hopefully this problem has been solved.")


def print_files_error_info(filename):
    print("---------------Debug info begins:--------------")
    print("Oops, we met a ValueError.")
    print("Please check this file " + filename)
    print(
        "1. A Possible reason is that there is a redundant comma behind last group of author/quote in this "
        "json file.")
